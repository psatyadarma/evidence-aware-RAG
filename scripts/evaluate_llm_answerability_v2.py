"""Run the single authorized live v2 evaluation on development data."""
...
from app.llm_answerability_v2 import (  # noqa: E402
...
from evaluation.llm_answerability_v2_evaluation import (  # noqa: E402
    evaluate_llm_answerability_v2,
    write_llm_answerability_v2_report,
)
...
REPORT_PATH = RESULTS_DIR / "development_llm_answerability_v2_report.jsonl"
SUMMARY_PATH = RESULTS_DIR / "development_llm_answerability_v2_summary.json"


def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the paid v2 hosted-model classification on development data."
    )
    parser.add_argument(
        "--live",
        action="store_true",
...
def main() -> None:
    ...
    if REPORT_PATH.exists() or SUMMARY_PATH.exists():
        raise SystemExit(
            "V2 result artifacts already exist; refusing to overwrite the single run."
        )
    settings = Settings()
    if settings.model_provider.casefold() != "openai":
        raise SystemExit(
            "Checkpoint 9 v2 supports exactly one provider: openai."
        )
    if settings.model_name != "gpt-4.1-mini-2025-04-14":
        raise SystemExit(
            "Checkpoint 9 v2 requires the pinned gpt-4.1-mini-2025-04-14 model."
        )
    if settings.model_api_key is None or not settings.model_api_key.get_secret_value():
        raise SystemExit("RAG_MODEL_API_KEY is required for the live v2 run.")
    ...
    llm = LlmEvidenceSufficiencyClassifierV2(
        retriever,
        provider,
    ...
    write_llm_answerability_v2_report(report, summary, REPORT_PATH, SUMMARY_PATH)
